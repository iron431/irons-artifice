...
public final class AmmoCountHudOverlay {
    ...
    public static void render(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        if (showIcon) {
            ...
            // 1.21.1 has no tinted blit, so tint via setColor around an explicit flush.
            graphics.flush();
            graphics.setColor(((iconColor >> 16) & 0xFF) / 255.0F, ((iconColor >> 8) & 0xFF) / 255.0F, (iconColor & 0xFF) / 255.0F, ((iconColor >> 24) & 0xFF) / 255.0F);
            graphics.blit(BULLET_ICON, Math.round(left), Math.round(iconY), 0.0F, 0.0F, iconSize, iconSize, iconSize, iconSize);
            graphics.flush();
    ...
    private static void renderFlash(GuiGraphics graphics, String flashText, Font font, float partialTick, boolean shadow, float startX, float startY, float baseScale, float travel, float endScale) {
        ...
        int color = withAlpha(alpha, ClientConfig.colorFlash());
    ...
    static int magazineTextColor(int loaded, int capacity) {
        ...
        return lerpArgb(t, ClientConfig.colorLow(), full);
    }

    private static int lerpArgb(float t, int from, int to) {

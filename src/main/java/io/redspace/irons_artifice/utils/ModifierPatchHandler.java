...
public final class ModifierPatchHandler {
    ...
    public static void rebuild(ItemStack gun, Container modifiers) {
        ...
        DataComponentPatch previous = gun.getOrDefault(DataComponentRegistry.MODIFIER_PATCH.get(), DataComponentPatch.EMPTY);
        ...
        if (next.isEmpty()) {
            gun.remove(DataComponentRegistry.MODIFIER_PATCH.get());
            return;
        }
        gun.applyComponents(next);
        gun.set(DataComponentRegistry.MODIFIER_PATCH.get(), next);
    }
